...
Goal: make ComplyScan work locally and on Cloudflare for free scans, full report access, and agency/monthly subscriptions, with a reliable automated E2E path.

Architecture:
- Make lib/env.ts support both Cloudflare D1 and local SQLite through one adapter.
- Normalize paid flows so legacy one-time and agency subscription both work in real mode and in deterministic mock-payment E2E mode.
...
3. Fix Stripe checkout/session/webhook flow for legacy one-time + agency subscription.
4. Fix dashboard/subscriber/client routes to await DB access and honor agency plan.
5. Lock down full-report access so unpaid users cannot bypass the upgrade flow.
